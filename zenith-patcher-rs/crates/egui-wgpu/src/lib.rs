...
impl RenderState {
    ...
    pub async fn create(
        ...
        let adapter = {
            ...
            let req_adapter = match req_adapter {
                Some(a) => Some(a),
                None => {
                    // Repli automatique sur l'adaptateur logiciel WARP / llvmpipe (essentiel pour les VM)
                    instance
                        .request_adapter(&wgpu::RequestAdapterOptions {
                            power_preference: config.power_preference,
                            compatible_surface: Some(surface),
